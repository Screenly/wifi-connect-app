...
Both workflows also accept an `edge_app_id` input (pinned to a specific `edge-apps-actions` commit ahead of its next tagged release, matching [`Screenly/airtable-app#16`](https://github.com/Screenly/airtable-app/pull/16)) sourced from an `EDGE_APP_ID` secret, falling back to `STAGE_EDGE_APP_ID` / `PRODUCTION_EDGE_APP_ID` repo variables per environment. Setting one of those skips the manifest-file id lookup entirely and stops branching between `screenly.yml`/`screenly_qc.yml` — useful once an Edge App already exists and you'd rather not depend on the committed manifest id. Leaving all three unset keeps the current per-environment-manifest behavior unchanged.
